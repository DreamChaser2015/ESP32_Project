...
#else
    lv_label_set_text(label, LV_SYMBOL_BELL" Hello world "LV_SYMBOL_BELL"\n "LV_SYMBOL_WARNING" Espressif and LVGL "LV_SYMBOL_WARNING);
#endif
...
static lv_disp_rotation_t rotation_current = LV_DISPLAY_ROTATION_0;
void lcd_set_rotation(lv_display_rotation_t rotation)
{
    if (rotation_current != rotation)
    {
        // TODO: 为什么要设置偏移才正常
        ...
        {
            esp_lcd_panel_set_gap(lcd_panel, 80, 0);
        }
        else
        {
...
void test_lvgl(void * arg)
{
    ...
    while (1)
    {
        ...
        lv_bar_set_value(bar, x / 1, LV_ANIM_ON);
    }
}
